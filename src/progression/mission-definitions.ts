...
export type MissionRule =
  | Readonly<{ kind: 'count_bets' }>
  | Readonly<{ kind: 'count_wins' }>
  | Readonly<{ kind: 'hit_multiplier'; multiplier: number }>
  | Readonly<{ kind: 'count_risk'; risk: Risk }>
  | Readonly<{ kind: 'wager_credits' }>
  | Readonly<{ kind: 'try_all_risks' }>;
...
export const DAILY_MISSIONS: readonly MissionDefinition[] = freezeMissionDefinitions([
...
export const STARTER_MISSIONS: readonly MissionDefinition[] = freezeMissionDefinitions([
...
function cloneMissionDefinition(definition: MissionDefinition): MissionDefinition {
  return {
    ...definition,
...
function freezeMissionDefinitions(definitions: MissionDefinition[]): readonly MissionDefinition[] {
  for (const definition of definitions) {
    Object.freeze(definition.rule);
