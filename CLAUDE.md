...
| Task | Skill |
|---|---|
| About to `git commit` anything that touches `src/`, `test/`, `prisma/`, `Dockerfile`, `fly.toml`, `.github/workflows/`, or `.claude/` | [.claude/skills/pre-commit/SKILL.md](.claude/skills/pre-commit/SKILL.md) |
| Pushing to `main`, manually running `flyctl deploy`, or investigating a prod incident | [.claude/skills/deploy-flow/SKILL.md](.claude/skills/deploy-flow/SKILL.md) |

## Path-scoped rules — read before editing

Before editing files under any of the paths below, **read the matching rule file**. Each rule contains the invariants for that subsystem; CLAUDE.md only carries cross-cutting context.

| When editing… | Read |
|---|---|
| `src/**/*.controller.ts`, `src/**/*.dto.ts`, `src/**/*.query.ts`, `src/**/*.response.ts`, `src/common/**`, `src/main.ts`, `test/e2e/**` | [.claude/rules/api.md](.claude/rules/api.md) |
| `src/auth/**`, `src/users/**`, `test/e2e/auth*.e2e-spec.ts` | [.claude/rules/auth.md](.claude/rules/auth.md) |
| `src/bets/**`, `src/wallet/**`, `src/game/**`, `test/e2e/bets*.e2e-spec.ts` | [.claude/rules/bets.md](.claude/rules/bets.md) |
| `src/seeds/**`, `src/game/engine.ts`, `src/game/types.ts`, `src/game/payout-tables.ts`, `test/e2e/seeds*.e2e-spec.ts` | [.claude/rules/seeds.md](.claude/rules/seeds.md) |
| `prisma/schema.prisma`, `prisma/migrations/**`, `src/prisma/**`, `src/**/*.service.ts` | [.claude/rules/prisma.md](.claude/rules/prisma.md) |

A file can match more than one rule (e.g. `src/bets/bets.service.ts` triggers both `bets` and `prisma`). Read all matches.

The `paths:` glob list lives in each rule's YAML frontmatter — it is the machine-readable source of truth and will later be consumed by `.claude/hooks/` and the source-to-doc mapping (see [docs/superpowers/plans/2026-05-19-ai-workflow.md](docs/superpowers/plans/2026-05-19-ai-workflow.md)). When adding a new rule, mirror the frontmatter shape and add a row to the table above.
...
|---|---|---|
| [src/auth](src/auth/) | Registration, login, JWT issuance, refresh-token rotation | [auth](.claude/rules/auth.md) |
| [src/users](src/users/) | `findByEmail` / `findById` / `createWithSeed` (user + initial seed in one tx) | [auth](.claude/rules/auth.md) |
| [src/seeds](src/seeds/) | Provably-fair server/client seed lifecycle (commit → bet → reveal/rotate) | [seeds](.claude/rules/seeds.md) |
| [src/wallet](src/wallet/) | Pure payout math + row-locking `lockAndApply` used inside the bet tx | [bets](.claude/rules/bets.md) |
| [src/game](src/game/) | Deterministic `play()` engine, payout tables, `GET /game/config` | [seeds](.claude/rules/seeds.md), [bets](.claude/rules/bets.md) |
| [src/bets](src/bets/) | `POST /bets` orchestrator that ties seeds + wallet + engine together | [bets](.claude/rules/bets.md) |
| [src/common](src/common/) | Global exception filter, BigInt interceptor, `GET /health` | [api](.claude/rules/api.md) |
| [src/prisma](src/prisma/) | `PrismaService` wrapper, `@Global()` module | [prisma](.claude/rules/prisma.md) |
...
See [.claude/rules/api.md](.claude/rules/api.md) for the contracts these enforce.
...
The exact payout math (fixed-point `* 10_000`) lives in [.claude/rules/bets.md](.claude/rules/bets.md).

### Swagger / DTO file naming

`nest-cli.json` enables the `@nestjs/swagger` CLI plugin with `dtoFileNameSuffix: ['.dto.ts', '.query.ts', '.response.ts']`. **OpenAPI schemas are auto-generated only for files matching those suffixes** — keep DTOs in `dto/` subfolders with those exact suffixes or they won't appear in `/docs`. Full convention in [.claude/rules/api.md](.claude/rules/api.md).
