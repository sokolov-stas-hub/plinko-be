...
- Does it touch `Bet`, `Seed`, `User`, or `RefreshToken` indexes? → Cross-check against [.Codex/rules/prisma.md](../../rules/prisma.md) — the listed indexes are load-bearing.
...
- Pre-commit checks: [.Codex/skills/pre-commit/SKILL.md](../pre-commit/SKILL.md) — must be clean before a deploy.
- Schema/migration invariants: [.Codex/rules/prisma.md](../../rules/prisma.md)
